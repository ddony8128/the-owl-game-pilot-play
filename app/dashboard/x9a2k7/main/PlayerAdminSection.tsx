...
export function PlayerAdminSection() {
  ...
  const reset = async (scope: "runtime" | "all") => {
    const confirmMsg =
      scope === "all"
        ? "이번 세션의 참가자 목록과 모든 게임 진행 상태를 초기화합니다.\n다음 팀을 받기 전 사용하는 기능입니다. 계속하시겠습니까?"
        : "참가자 목록은 유지하고 모든 게임 진행 상태만 초기화합니다. 계속하시겠습니까?";
    ...
    <section className="space-y-3 text-sm">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder="닉네임 입력"
          className="h-8 w-44 rounded border border-zinc-700 bg-zinc-900 px-2 text-sm outline-none focus:border-zinc-400"
        />
        <button
          type="button"
          disabled={busy || !newName.trim()}
          onClick={() => void addPlayer()}
          className="h-8 rounded bg-amber-400 px-3 text-sm font-semibold text-zinc-950 hover:bg-amber-300 disabled:opacity-40"
        >
          참가자 등록
        </button>
        <span className="text-sm text-zinc-500">총 {players.length}명</span>
      </div>
          ...
          players.map((p) => (
            ...
            >
              ...
              <button
                ...
                className="h-6 rounded border border-red-500/40 px-2 text-sm text-red-300 hover:bg-red-500/10 disabled:opacity-40"
              >
                제거
              </button>
            </div>
      ...
      <div className="space-y-2 rounded-lg border border-zinc-800 bg-zinc-900 p-3">
        <h3 className="text-sm font-semibold text-zinc-300">세션 종료 / 다음 팀 준비</h3>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            disabled={busy}
            onClick={() => void reset("runtime")}
            className="h-8 rounded border border-orange-400/60 px-3 text-sm text-orange-300 hover:bg-orange-500/10 disabled:opacity-40"
          >
            진행 상태만 초기화 (참가자 유지)
          </button>
          <button
            ...
            className="h-8 rounded bg-red-600 px-3 text-sm font-semibold text-white hover:bg-red-500 disabled:opacity-40"
          >
