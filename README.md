...
구현 파일: `app/vote/page.tsx`

- `PageGuard`로 보호: `requireLogin`, `allowGames={["vote"]}`
- 플레이어 2명 × 2세트 선택
  - 클라이언트는 `GET /api/vote/players`를 통해 `players` 목록을 받아와 셀렉트 박스로 표시
  - 자기 자신은 목록에서 제외
- 이유 textarea(최소 5글자)
- 제출 시
...
구현 파일: `app/quizshow/page.tsx`

- `PageGuard`로 보호: `requireLogin`, `allowGames={["quiz"]}`, `requireFinalist`
- 초기 로딩 시
